...
class Program
{
    ...
    static void LocalFileSender()
    {
        ...
        Console.Write("Enter the path of the local file to send: ");
        string filePath = Console.ReadLine();
        ...
        try
        {
            ...
            Console.WriteLine($"Local IP Address: {localIpAddress}, Port: {port}");

            Console.WriteLine("Waiting for a receiver to connect...");

            TcpClient receiverClient = listener.AcceptTcpClient();
            Console.WriteLine("Receiver connected.");

            NetworkStream networkStream = receiverClient.GetStream();
            ...
            // Close the TcpClient and TcpListener
...
using System.Text;
using System.Threading.Tasks;
using System.Timers; // Use the System.Timers namespace for Timer
            ...
            try
            {
                using (TcpClient senderClient = new TcpClient(receiverIpAddress, port))
                {
                    ...
                    using (FileStream fileStream = File.OpenRead(filePath))
                    {
                        ...
                        long totalBytesSent = 0;
                        DateTime startTime = DateTime.Now;
                        System.Timers.Timer speedTimer = new System.Timers.Timer(1000); // Use System.Timers.Timer
                        speedTimer.Elapsed += (sender, e) =>
                        {
                            TimeSpan elapsedTime = DateTime.Now - startTime;
                            double speed = totalBytesSent / elapsedTime.TotalSeconds;
